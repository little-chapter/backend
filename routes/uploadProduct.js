  ...
  async (req, res) => {
    ...
    try {
      ...
      const failedUploads = uploadResults.filter(
        (result) => result.status === "rejected" || !result.value.status
      );
      ...
      // 準備要插入的圖片資料
      const imageDataToInsert = successfulUploads.map(
        (uploadResult, index) => ({
      ...
      const otherImages = successfulUploads.filter(
        (_, index) => index !== isPrimary
      );
